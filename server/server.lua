...
QBCore.Functions.CreateCallback('fivem-appearance:getPlayerSkin', function(source, cb)
	...
	local players = MySQL.query.await('SELECT skin FROM players WHERE citizenid = ?', {Player.PlayerData.citizenid})
...
RegisterNetEvent('fivem-appearance:getOutfit', function(name)
	...
	local outfit = MySQL.query.await('SELECT outfit FROM player_outfits WHERE citizenid = @citizenid AND name = @name', {['@citizenid'] = Player.PlayerData.citizenid, ['@name'] = name})
...
RegisterNetEvent('fivem-appearance:getOutfits', function()
	...
	local Player = QBCore.Functions.GetPlayer(src)
	local myOutfits = {}
	local result = MySQL.query.await('SELECT id, name, ped, components, props FROM player_outfits WHERE citizenid = ?', {Player.PlayerData.citizenid})
	for i=1, #result, 1 do
...
AddEventHandler('fivem-appearances:buyclothing', function()
	...
	if Config.Money < 0 then
		if Player.Functions.RemoveMoney('cash', Config.Money, "Clothing Purchase") then
